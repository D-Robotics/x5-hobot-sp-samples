...
class Resnet18:
    ...
    def pre_process(self, img , resize_type: int = 1):
        """
        @brief Preprocess the input image into model-acceptable NV12 format.

        @param img (np.ndarray) Input image in BGR format.
        @param resize_type (int) Resize algorithm type (default = 1).
        @return dict: Nested input dict: {model_name: {input_name: input_tensor}}
        """
    ...
    def post_process(self,
                    ...
                    topk: int = 5) -> None:
        """
        @brief Use the model output directly for ranking, and remove redundant softmax.
        """
        # Extract the model output and compress redundant dimensions.
        prob = np.squeeze(outputs[self.output_names[0]])
        
        try:
            output_quanti_dict = self.model.output_quants[self.model_name]
            target_quant_info = output_quanti_dict[self.output_names[0]]
            prob = post_utils.dequantize_tensor(prob, target_quant_info)
        except:
            pass
...
def main() -> None:
    ...
    #               "rdk_model_zoo/rdk_x5/ResNet/resnet18_224x224_nv12.bin")
