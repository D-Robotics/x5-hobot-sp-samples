...
本示例基于 Ultralytics YOLO11 模型，通过 `HB_HBMRuntime` 接口完成图像的目标检测。支持图像预处理、推理、后处理（包含解码、置信度过滤、NMS）以及结果图像保存。
...
| 参数名           | 说明                                                         | 默认值                                         |
|------------------|--------------------------------------------------------------|------------------------------------------------|
| `--model-path`    | 模型文件路径（.bin 格式）                                    | `/opt/hobot/model/x5/basic/yolo11n_detect_nashe_640x640_nv12.bin`        |
...
- 运行模型
    ...
    - 指定参数运行
        ```bash
        python ultralytics_yolo11.py \
            --model-path yolo11n_detect_nashe_640x640_nv12.bin \
