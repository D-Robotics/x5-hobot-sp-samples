...
class YoloV8:
    """
    @brief YoloV8 object detection wrapper using HB_HBMRuntime backend.
    ...
    def __init__(self, opt):
        """
        @brief Initialize the YoloV8 model and parameters.
        ...
        # Input shape (H, W) - 使用第一个代码的维度获取方式
        ...
        self.resize_type = 1  # 使用第一个代码的resize_type
        self.classes_num = 80
        self.reg = 16
    ...
    def post_process(self,
        ...
        """
        @brief 使用第二个代码的后处理方法
        Postprocess YoloV8 model outputs to obtain final detection results.
        ...
        # Loop through detection branches (for different scales)  这里是为了获取每一层decode的结果,问题应该是出在这里
...
def main() -> None:
    """
    @brief YoloV8 object detection sample.
    ...
    parser.add_argument('--model-path', type=str,
                        default='yolov8x_detect_bayese_640x640_nv12.bin',
    ...
    if not os.path.exists(opt.model_path):
        print(f"[Warning] File {opt.model_path} does not exist. Please download YoloV8 model manually.")
        return
    ...
    yolov8.set_scheduling_params(priority=opt.priority, bpu_cores=opt.bpu_cores)

    # Print model metadata (input/output names, shapes, etc.)
    common.print_model_info(yolov8.model)

    # Load input image
    img: np.ndarray = common.load_image(opt.test_img)
    img_h, img_w = img.shape[:2]

    # Preprocess input image into model-compatible NV12 format (使用第一个代码的前处理)
    input_array = yolov8.pre_process(img)

    # Run forward inference
    outputs = yolov8.forward(input_array)

    # Postprocess: decode + filter + NMS + rescale to original resolution (使用第二个代码的后处理)
    boxes, cls_ids, scores = yolov8.post_process(outputs, img_w, img_h)
