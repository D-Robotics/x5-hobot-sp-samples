...
class YoloV10:
    ...
    def post_process(self,
        ...
        """
        @brief 使用第二个代码的后处理方法
        Postprocess YOLOv11 model outputs to obtain final detection results.
        ...
        # xyxy = postprocess.scale_coords_back(
            dbboxes, img_w, img_h, self.input_W, self.input_H, self.resize_type)

        # return xyxy, ids[keep], scores[keep]
        return xyxy, ids, scores
...
def main() -> None:
    ...
    parser.add_argument('--model-path', type=str,
                        default='yolov10x_detect_bayese_640x640_nv12.bin',
    ...
    if not os.path.exists(opt.model_path):
        print(f"[Warning] File {opt.model_path} does not exist. Please download yolov10 model manually.")
        return

    # Instantiate yolov10 model
    yolov10 = YoloV10(opt)

    # Set BPU scheduling (core binding and priority)
    yolov10.set_scheduling_params(priority=opt.priority, bpu_cores=opt.bpu_cores)

    # Print model metadata (input/output names, shapes, etc.)
    common.print_model_info(yolov10.model)
    ...
    input_array = yolov10.pre_process(img)

    # Run forward inference
    outputs = yolov10.forward(input_array)

    # Postprocess: decode + filter + NMS + rescale to original resolution (使用第二个代码的后处理)
    boxes, cls_ids, scores = yolov10.post_process(outputs, img_w, img_h)
