...
本项目包含多个基于 Python 编写的 AI 示例程序，适用于 RDK 平台，覆盖图像分类、目标检测、实例分割、姿态估计、OCR、语音识别等常见 AI 任务。示例使用 `.bin` 格式的量化模型进行推理，便于开发者快速验证模型效果并开展应用开发。
...
### 其他组件
- hbm_rumtime：用于加载和运行`.bin`模型，系统默认已安装，如下自行安装可参考`4.1 Python 接口`部分。
...
+ 运行模型
    ...
    ```
    python resnet18.py \
    --model-path /opt/hobot/model/x5/basic/resnet18_224x224_nv12.bin \
    --test-img /app/pydev_demo/01_classification_sample/01_resnet18/zebra_cls.jpg \
    --label-file /app/pydev_demo/01_classification_sample/01_resnet18imagenet1000_clsidx_to_labels.txt
    ```
+ 查看结果
    ``` bash
    Top-5 Predictions:
    ...
    5: Class 343: 0.0004
    ```
    检查 01_resnet18imagenet1000_clsidx_to_labels.txt 的内容，可以看到  340: 'zebra', 这样的字段。
...
## 附加说明
* 所有示例程序均使用`.bin`格式模型，需配合平台`hbm_runtime`的python推理接口使用。
