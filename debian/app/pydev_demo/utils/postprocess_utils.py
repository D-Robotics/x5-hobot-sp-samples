...
def get_bounding_boxes(
        dilated_polys: Sequence[np.ndarray], min_area: int
    ) -> List[np.ndarray]:
        """
        @brief Get bounding boxes from dilated polygons.

        @param dilated_polys List of dilated polygons.
        @param min_area Minimum valid box area.
        @return List of 4-point bounding boxes.
        """
...
def dilate_contours(contours: Sequence[np.ndarray]) -> List[np.ndarray]:
        """
        @brief Dilate contours using the configured ratio_prime.

        @param contours List of contours from OpenCV.
        @return List of dilated integer polygon contours.
        """
        dilated_polys: List[np.ndarray] = []
        for poly in contours:
            ...
            if (
                dilated_poly.size == 0
                or dilated_poly.dtype != np.int_
                or len(dilated_poly) != 1
            ):
                continue
...
def dequantize_outputs(outputs: dict, quan_infos: dict) -> dict:
    ...
    for name, output in outputs.items():
        ...
        fp32_outputs[name] = dequantize_tensor(output, quant_info)  # 没有做反量化
    return fp32_outputs
